...
INSERT_WATCHED_MOVIE = (
    "INSERT INTO watched (user_username, movie_id)" " VALUES (%s,%s);"
)
SET_MOVE_WATCHED = "UPDATE movies SET watched = 1 WHERE title = %s;"
...
def watch_movie(user, movie_id):
    with connection:
        with connection.cursor() as cursor:
            cursor.execute(INSERT_WATCHED_MOVIE, (user, movie_id,))
